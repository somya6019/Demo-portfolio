...
export const SectorsMaster = () => {
  ...
  const toggleMutation = useToggleSector();
  ...
  const {
    ...
    handleFiltersChange,
    clearFilters,
  } = useDataTableManager<Sector>(data);

  /** ---------------- Table Config ---------------- */
  const tableConfig: ReusableDataTableConfig<Sector> = useMemo(
    () => ({
      columns: [
  ...
  /** ---------------- Row Actions ---------------- */
  const rowActions: RowAction<Sector>[] = useMemo(
    () => [
      {
        icon: "pi pi-pencil",
        label: "Edit",
  ...
  function handleEdit(row: Sector) {
    setFormData({
      name: row.name,
  ...
  async function handleDelete(row: Sector) {
    if (confirm(`Delete ${row.name}?`)) {
      await deleteMutation.mutateAsync(row.id);
    }
  }

  async function handleToggle(row: Sector) {
  ...
  return (
    <div className="p-4">
      ...
      <Dialog
        header={editingId ? "Edit Sector" : "Add Sector"}
        visible={showDialog}
        onHide={() => {
          setShowDialog(false);
