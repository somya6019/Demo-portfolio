...
import {
  ...
  useDeleteOccurrence,
  useToggleOccurrence,
} from "@/hooks/master/useOccurrences";
...
export const OccurrencesMaster = () => {
  ...
  const toggleMutation = useToggleOccurrence();
  ...
  const {
    ...
    handleFiltersChange,
    clearFilters,
  } = useDataTableManager<Occurrence>(data);

  /** ---------------- Table Config ---------------- */
  const tableConfig: ReusableDataTableConfig<Occurrence> = useMemo(
    () => ({
      columns: [
        { field: "id", header: "ID", width: "5%", filterType: "none" },
        {
          field: "name",
          header: "Occurrence Name",
          filterType: "text",
  ...
  /** ---------------- Row Actions ---------------- */
  const rowActions: RowAction<Occurrence>[] = useMemo(
    () => [
      {
        icon: "pi pi-pencil",
        label: "Edit",
  ...
  function handleEdit(row: Occurrence) {
    setFormData({
      name: row.name,
  ...
  async function handleDelete(row: Occurrence) {
    if (confirm(`Delete ${row.name}?`)) {
      await deleteMutation.mutateAsync(row.id);
    }
  }

  async function handleToggle(row: Occurrence) {
  ...
  return (
    <div className="p-4">
      ...
      <Dialog
        header={editingId ? "Edit Occurrence" : "Add Occurrence"}
        visible={showDialog}
        onHide={() => {
          setShowDialog(false);
