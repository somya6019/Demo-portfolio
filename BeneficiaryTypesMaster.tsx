...
import {
  ...
  useToggleBeneficiaryType,
} from "@/hooks/master/useBeneficiaryTypes";
...
export const BeneficiaryTypesMaster = () => {
  const { data = [], isLoading } = useBeneficiaryTypes();
  const createMutation = useCreateBeneficiaryType();
  const updateMutation = useUpdateBeneficiaryType();
  const deleteMutation = useDeleteBeneficiaryType();
  const toggleMutation = useToggleBeneficiaryType();
  ...
  const {
    ...
    handleFiltersChange,
    clearFilters,
  } = useDataTableManager<BeneficiaryType>(data);

  /** ---------------- Table Config ---------------- */
  const tableConfig: ReusableDataTableConfig<BeneficiaryType> = useMemo(
    () => ({
      columns: [
      ...
      emptyMessage: "No Beneficiary Types found.",
    }),
    []
  );

  /** ---------------- Row Actions ---------------- */
  const rowActions: RowAction<BeneficiaryType>[] = useMemo(
    () => [
      {
        icon: "pi pi-pencil",
        label: "Edit",
    ...
    try {
      if (editingId) {
        ...
        toastRef.current?.show({
          severity: "success",
          summary: "Updated",
          detail: "Beneficiary Type updated",
        });
      } else {
        ...
        toastRef.current?.show({
          severity: "success",
          summary: "Created",
          detail: "Beneficiary Type created",
        });
      }
      resetForm();
      setShowDialog(false);
  ...
  function handleEdit(row: BeneficiaryType) {
    setFormData({
      name: row.name,
  ...
  async function handleDelete(row: BeneficiaryType) {
    if (confirm(`Delete ${row.name}?`)) {
      await deleteMutation.mutateAsync(row.id);
    }
  }

  async function handleToggle(row: BeneficiaryType) {
  ...
  return (
    <div className="p-4">
      ...
      <Dialog
        header={editingId ? "Edit Beneficiary Type" : "Add Beneficiary Type"}
        visible={showDialog}
        onHide={() => {
          setShowDialog(false);
      ...
      >
        <form onSubmit={handleSubmit}>
          <div className="mb-3">
            <label>Beneficiary Type Name *</label>
            <InputText
              name="name"
              value={formData.name}
